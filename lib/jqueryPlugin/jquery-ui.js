        ...
        a.fn.extend({
            ...
            disableSelection: function () {
                return this.bind((a.support.selectstart ? "selectstart" :
                    "mousedown") + ".ui-disableSelection", function (h) {
                    h.preventDefault()
                })
            },
...
(function (a, d) {
    if (a.cleanData) {
        ...
        a.fn.remove = function (h, g) {
            return this.each(function () {
                if (!g)
                    if (!h || a.filter(h, [this]).length) a("*", this).add([this]).each(function () {
                        try {
    ...
    a.widget.bridge = function (h, g) {
        a.fn[h] = function (i) {
            ...
            b ? this.each(function () {
                var l = a.data(this, h),
                    o = l && a.isFunction(l[i]) ? l[i].apply(l, f) : l;
                if (o !== l && o !== d) {
                    j = o;
                    return false
                }
        ...
        _generatePosition: function (d) {
            ...
            if (this.originalPosition) {
                ...
                if (this.containment) {
                    ...
                    } else b = this.containment; if (d.pageX - this.offset.click.left < b[0]) g = b[0] + this.offset.click.left;
            ...
            for (var e = a(this).data("draggable"), h = e.options, g = h.snapTolerance, i = c.offset.left, b = i + e.helperProportions.width, f = c.offset.top, j = f + e.helperProportions.height, l = e.snapElements.length - 1; l >= 0; l--) {
                ...
                if (o - g < i && i < n + g && k - g < f && f < m + g || o - g < i && i < n + g && k - g < j && j < m + g || o - g < b && b < n + g && k - g < f && f < m + g || o - g < b && b < n + g && k - g < j &&
                    ...
                    if (!e.snapElements[l].snapping &&
                        (p || q || s || r || u)) e.options.snap.snap && e.options.snap.snap.call(e.element, d, a.extend(e._uiHash(), {
    ...
    a.widget("ui.resizable", a.ui.mouse, {
        ...
        _create: function () {
            ...
            if (this.handles.constructor == String) {
                ...
                for (var i = 0; i < g.length; i++) {
                    var b = a.trim(g[i]),
                        f = a('<div class="ui-resizable-handle ' + ("ui-resizable-" + b) + '"></div>');
            ...
            this._renderAxis = function (j) {
                ...
                for (var l in this.handles) {
                    ...
                    if (this.elementIsWrapper && this.originalElement[0].nodeName.match(/textarea|input|select|button/i)) {
                        var o = a(this.handles[l], this.element),
                            n = 0;
        ...
        _updateRatio: function (e) {
            var h = this.position,
                g = this.size,
                i = this.axis;
            if (c(e.height)) e.width = e.height * this.aspectRatio;
            else if (c(e.width)) e.height = e.width / this.aspectRatio;
            if (i == "sw") {
    ...
    a.ui.plugin.add("resizable", "alsoResize", {
        ...
        resize: function (e, h) {
                ...
                f = {
                    ...
                    a(l).each(function () {
                        var n = a(this),
                            ...
                            m = {}, p = o && o.length ? o : n.parents(h.originalElement[0]).length ? ["width", "height"] : ["width", "height", "top", "left"];
            ...
            typeof e.alsoResize == "object" && !e.alsoResize.nodeType ?
                a.each(e.alsoResize, function (l, o) {
                    j(l, o)
                }) : j(e.alsoResize)
        },
        stop: function () {
...
(function (a) {
    a.widget("ui.sortable", a.ui.mouse, {
            ...
            for (c = this.items.length - 1; c >= 0; c--) {
                ...
                if (g)
                    if (h != this.currentItem[0] && this.placeholder[g == 1 ? "next" : "prev"]()[0] != h && !a.ui.contains(this.placeholder[0], h) && (this.options.type == "semi-dynamic" ? !a.ui.contains(this.element[0],
                        ...
                        if (this.options.tolerance == "pointer" || this._intersectsWithSides(e)) this._rearrange(d, e);
                        else break;
        ...
        serialize: function (d) {
            var c = this._getItemsAsjQuery(d && d.connected),
                e = [];
            d = d || {};
            a(c).each(function () {
        ...
        _getItemsAsjQuery: function (d) {
            ...
            if (h && d)
                for (d = h.length - 1; d >= 0; d--)
                    for (var g = a(h[d]), i = g.length - 1; i >= 0; i--) {
        ...
        _removeCurrentsFromItems: function () {
            for (var d = this.currentItem.find(":data(sortable-item)"), c = 0; c < this.items.length; c++)
        ...
        _refreshItems: function (d) {
            ...
            if (h)
                for (var g = h.length - 1; g >= 0; g--)
                    for (var i = a(h[g]), b = i.length - 1; b >= 0; b--) {
        ...
        _clear: function (d, c) {
            ...
            if (!a.ui.contains(this.element[0], this.currentItem[0])) {
                ...
                for (h = this.containers.length - 1; h >= 0; h--)
                    if (a.ui.contains(this.containers[h].element[0], this.currentItem[0]) && !c) {
                        e.push(function (g) {
                            return function (i) {
...
jQuery.effects || function (a, d) {
    ...
    function h() {
        ...
        if (n && n.length && n[0] && n[n[0]])
            ...
            for (m in n)
                if (typeof n[m] === "string") k[m] = n[m]; return k
    }
    ...
    a.fn.extend({
        ...
        toggleClass: function (n, k, m, p, q) {
            return typeof k == "boolean" || k === d ? m ? a.effects.animateClass.apply(this, [k ? {
                ...
                m, p, q
            ]) : this._toggleClass(n, k) : a.effects.animateClass.apply(this, [{
                toggle: n
            },
                k, m, p
            ])
    ...
    a.extend(a.easing, {
        ...
        easeInOutElastic: function (n, k, m, p, q) {
            ...
            if (r < Math.abs(p)) {
                r = p;
                n = s / 4
            } else n = s / (2 * Math.PI) * Math.asin(p / r); if (k < 1) return -0.5 * r * Math.pow(2, 10 * (k -= 1)) * Math.sin((k * q - n) * 2 * Math.PI / s) + m;
        ...
        easeOutBounce: function (n, k, m, p, q) {
            return (k /= q) < 1 / 2.75 ? p * 7.5625 * k * k + m : k < 2 / 2.75 ? p * (7.5625 * (k -= 1.5 / 2.75) * k + 0.75) + m : k < 2.5 / 2.75 ? p * (7.5625 * (k -= 2.25 / 2.75) * k + 0.9375) + m : p * (7.5625 * (k -= 2.625 / 2.75) * k + 0.984375) + m
        },
    ...
    a.effects.explode = function (d) {
        return this.queue(function () {
            ...
            for (var i = h.outerWidth(true), b = h.outerHeight(true), f = 0; f < c; f++)
                for (var j =
                    0; j < e; j++) h.clone().appendTo("body").wrap("<div></div>").css({
                    position: "absolute",
                    visibility: "visible",
                    left: -j * (i / e),
                    top: -f * (b / c)
    ...
    a.extend(c.prototype, {
        ...
        _autoSize: function (b) {
            if (this._get(b, "autoSize") && !b.inline) {
                ...
                if (j.match(/[DM]/)) {
                    var l = function (o) {
                        for (var n = 0, k = 0, m = 0; m < o.length; m++)
                            if (o[m].length > n) {
                                n = o[m].length;
                                k = m
                            }
    ...
    a.extend(a.ui.dialog.overlay, {
        ...
        create: function (g) {
            ...
            var i = (this.oldInstances.pop() ||
                a("<div></div>").addClass("ui-widget-overlay")).appendTo(document.body).css({
                    width: this.width(),
                    height: this.height()
                });
