...
Camera = function(){
    ...
    return {
        ...
        rotateX: function( dx ){
            ...
            quaternMatrix4 = mQuaternion.getMatrix();
            transformed = true;
            var ang = mQuaternion.getEulerAngleSub1();
            angle.setX( angle.getX() + dx );
            console.debug( 'roto*', parseInt(angle.getX())+':'+parseInt(angle.getY())+':'+parseInt(angle.getZ()));
            console.debug( 'rota^', parseInt(ang.getX())+':'+parseInt(ang.getY())+':'+parseInt(ang.getZ()));
            ang = mQuaternion.getEulerAngle2();
            console.debug( 'rotav', parseInt(ang.getX())+':'+parseInt(ang.getY())+':'+parseInt(ang.getZ()));
        },
        rotateY: function( dy ){
            var yq = quaternion().fromAxisAngle( vector3(0, 1, 0), dy );
            mQuaternion = yq.crossBy( mQuaternion );
            quaternMatrix4 = mQuaternion.getMatrix();
            angle.setY( angle.getY() + dy );
            var ang = mQuaternion.getEulerAngleSub1();
